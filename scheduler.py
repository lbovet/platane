            ...
            for item in all_items.values():            
                ...
                if sub_item.startswith(orig_name):
                    ...
                    if round(item['effort'],3) != round(sum(s[sub_item]),3):
                        result.append( (sub_item, s[sub_item], all_items[sub_item]['effort'], sum(s[sub_item]), task_dict[orig_name] ))
                        continue
                    summed_effort = summed_effort+item['effort']
                    for i in range(len(slots)):
                        summed_slots[i]=summed_slots[i]+s[sub_item][i]
            result.append( (orig_name, summed_slots, summed_effort, sum(summed_slots), task_dict[orig_name] ))                       
        else:
...
def render(tasks, vars={}):
    ...
    start, slots, sched = schedule_tasks(good_tasks)
